...
export async function createValidationOfferToro(data) {
    ...
    const user_id = data.body.user_id ? data.body.user_id : data.query.user_id;
    const amount = data.body.amount ? data.body.amount : data.query.amount;
    const o_name = data.body.o_name ? data.body.o_name : data.query.o_name; // Add this line to get the offer name
    ...
    return await executeQuery({
        ...
        values: [
            user[0].hashId,
            "OfferToro",
            o_name, // Use the o_name parameter as the offer name
            montantRev,
