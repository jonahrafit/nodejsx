...
export async function getCommandByUserOrderByDateDesc(userID) {
    try {
        const res = await executeQuery({
            query: `SELECT commandes.status,commandes.code_promo, commandes.amount,boutique.nom as bnom,boutiqueCategorie.nom as cnom, commandes.date   FROM commandes INNER JOIN boutique ON commandes.shopID = boutique.id INNER JOIN boutiqueCategorie ON boutiqueCategorie.id = boutique.categorieId WHERE commandes.userID = ? ORDER BY STR_TO_DATE(date, '%m/%d/%Y') DESC `,
            values: [userID],
        });
        return res;
    } catch (error) {
